...
/**
 * Injects a copy button into every `pre` block (with a `code` child) inside
 * `container`. Idempotent — skips blocks that already have a button.
 */
export function injectCopyButtons(container: HTMLElement): void {
  const blocks = container.querySelectorAll<HTMLPreElement>("pre:has(> code)")
